...
router.post('/changepass', (req, res)=> {
    ...
    if(req.body.password == req.body.oldPassword){
        ...
        res.render('customer/change-password', {errs: [{message: "passsword does not match!"}], res: [], success: []});
    }
...
router.post('/books', (req, res)=> {
    ...
    bookModel.customerSearch(word, (result)=> {
        if(!result){
        ...
        if(!errors){
            bookModel.bookRequest(req.session.customer, data, (result)=> {
                if(result.length == 0){
                    ...
                    res.render('customer/books-request', {errs: [], success: [{message: "Your request has submitted thank you!"}]});
                }
            });
        }
        else {
